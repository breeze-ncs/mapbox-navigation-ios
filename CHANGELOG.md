...
* `RouteControllerMaximumDistanceBeforeRecalculating`, `RouterDelegate.router(_:shouldPreventReroutesWhenArrivingAt:)`, `NavigationServiceDelegate.navigationService(_:shouldPreventReroutesWhenArrivingAt:)` and `NavigationViewController.navigationViewController(_:shouldPreventReroutesWhenArrivingAt:)` are now deprecated and only affect `LegcyRouteController` behavior. ([#3754](https://github.com/mapbox/mapbox-navigation-ios/pull/3754))
